(function () {
    var kCart = function (jQuery, api_call) {
        ...
        obj.getCoupons=function (data,keycode){
            if(typeof data !="undefined"){
                if(data.hasOwnProperty("result")){
                    if(data.result.toLowerCase()=="success"){                  
                        ...
                        return {                               
                                "couponCode":target[index].couponCode,
                                "campaignProductId":target[index].campaignProductId
                               };
                    }else{
                        return false;
                    }                   
        ...
        //giadd ni sya para kuhaon ang tanan nga coupon codes tas iadd as property sa cart items
        //para every time mag apply sya coupon tas mag equal sya sa whole order ang whole order maapply pra sa discount
        obj.getWholeOrderCoupons =function(data){
            //this will get the whole order coupon code from the setup.
            var getWholeOrderCoupon=data.message.data[api.getCompaignId()].coupons.filter((coupon) => {
                    return coupon.campaignProductId === null;
            });

            var wholeOrderCoupons =[];
            for(var i in getWholeOrderCoupon){
                wholeOrderCoupons.push(getWholeOrderCoupon[i].couponCode)
            }

            return wholeOrderCoupons;
            
        }

        //get all product carted to apply whole order coupon
        //kani nga function magreturn ni sa mga cart items nga naa naay wholeorder nga property
        //pwd sad ka dire magkuha sa totalAmount sa giorder og sa shipping fee
        obj.wholeOrderData= function (arr){
            var data = [];
            var inc = 1;
            var totalPriceAmount=0;
            var totalShippingAmount=0;
            var totalPriceAllProd=0;
            var totalShippingAllProd=0;
            for(var i in arr){
                if(arr[i].hasOwnProperty("wholeOrderCoupons")){
                    var pushData = {};
                    pushData['product' + inc + '_id'] = arr[i].keycode;                  
                    pushData['product' + inc + '_qty'] = arr[i].qty;
                    data.push(pushData);
                    totalPriceAmount+=parseFloat(arr[i].prodPrice);
                    totalShippingAmount+=parseFloat(arr[i].shippingPrice);
                    inc++;
                }
                totalPriceAllProd+=parseFloat(arr[i].prodPrice);
                totalShippingAllProd+=parseFloat(arr[i].shippingPrice);
            }  
            return {
                        data: data,
                        totalShippingAmount: totalShippingAmount.toFixed(2),
                        totalPriceAmount : totalPriceAmount.toFixed(2),
                        totalPriceAllProd : totalPriceAllProd.toFixed(2),
                        totalShippingAllProd : totalShippingAllProd.toFixed(2)
                   }
        }

        obj.applyWholeOrderCoupon=function(coupon, arr, totalPriceAmount, totalShippingAmount){
            var wholeData=obj.wholeOrderData(arr).data;
            api.orderCouponWholeOrder(coupon,wholeData,function(data){                        
                    ...
                    for(var i in arr)  {
                        if(arr[i].hasOwnProperty("wholeOrderCoupons")){                            
                                var _data = {
                                            ...
                                            discounted: true,  
                                            couponCode:coupon, 
                                            wholeOrderApplied: true
                                } 
                                $.Topic("Updatecart").publish(_data);                             
                        }
                    }                  
        ...
        //ang gamit ani nga function kun naa nay coupon nga gingamit tas magadd og lain nga order
        //iya ra ireapply para maapil sa discount
        obj.reApplyCoupon=function(){
            var arr = JSON.parse(konnekCart.cartStorage()); 
            var couponCode="";
            ...
            for(var i in arr){
                if(typeof arr[i].coupon !="undefined"){
                        if(arr[i].coupon.hasOwnProperty("discounted") ){
                             couponCode=arr[i].coupon.couponCode;
                             discounted = arr[i].coupon.discounted;
                             break;
                        }
                }
            }
            if(discounted){
                obj.applyCouponToOrder(couponCode, false);
            }
     
        }

        //nagvalidate rani sa coupon kun per product ang basahon
        obj.validateCoupon=function (coupon,productIdWithCoupon,qty) {
        ...
        //mao ni nga coupon ang tawgon kun magclick ang customer sa apply coupon nga button
        obj.applyCouponToOrder=function(coupon, fromButtonClick){
            var btnCouponClick = typeof fromButtonClick =="undefined" ? false: fromButtonClick;
            var arr = JSON.parse(obj.cartStorage()); 
            var totalPriceAmount=obj.wholeOrderData(arr).totalPriceAmount;
            var totalShippingAmount=obj.wholeOrderData(arr).totalShippingAmount;
            
            //if wholeOrder or totalAmount mag base ang coupon kani maexecute
            if(typeof arr[0].wholeOrderCoupons!="undefined" && arr[0].wholeOrderCoupons.indexOf(coupon.toUpperCase())!=-1){
                    obj.applyWholeOrderCoupon(coupon, arr, totalPriceAmount, totalShippingAmount);
            }else{
            //kani kun maexecute kun ang coupon per product maapply
                    var cartWithCoupon=0;
                    for(var i=0; i<arr.length; i++){ 
                         if(arr[i].hasOwnProperty('coupon')){
                            if(!arr[i].coupon.hasOwnProperty('discounted')){
                                obj.validateCoupon(coupon,arr[i].keycode,arr[i].qty).then(function(data){  
                                    if(data!=""){
                    ...
                    if(cartWithCoupon==0 && btnCouponClick){
                        alert("Invalid Coupon Code");
        ...
        obj.cartEvents=function (){            
            ...
            $(".r_addToCart").unbind().on("click",  function () {
                ...
                data.wholeOrderCoupons = obj.getWholeOrderCoupons(api.getkonnecktiveSetup());//get whole order coupons
                if(typeof obj.getCoupons(api.getkonnecktiveSetup(), keycode)=="object"){                  
                    data.coupon=obj.getCoupons(api.getkonnecktiveSetup(), keycode);
                }else if(data.wholeOrderCoupons.length==0){
                        data.noCoupon=true;
                }
            ...
            $(".btnCoupon").unbind().on("click", function (e) {
                ...
                if(coupon!=""){
                     if (!!obj.cartStorage()) {  
                                        obj.validateCoupon(coupon,$(this).attr("keycode"),$(this).attr("qty")).then(function(data){  
                                            if(data!=""){
                                                $.Topic("Updatecart").publish(data); 
            ...
            //single coupon code container in cart summary
            $("#btnCoupon").unbind().on("click", function (e) {
                e.stopPropagation();
                $(this).html('<img src="/onlineorder/images/loading btn.svg" width="40" height="40" class="img-fluid inline-block" />');
                ...
                if(coupon!=""){
                     if (!!obj.cartStorage()) {
                        obj.applyCouponToOrder(coupon, true);
                     }else{
                         alert("Invalid Coupon Code");
                ...
                if (!isNaN(currentVal) && currentVal > 0) {
                    if(obj.quantityAssignPerProd){
                        ...
                        if (typeof current_obj != "undefined") {              
                            ...
                            var curPrice = (parseFloat(current_obj.price));
                            parent.find(".default-price span").html("$"+numberWithCommas(curPrice.toFixed(2)));
                            parent.find(".price").html("$"+numberWithCommas(curPrice.toFixed(2)));
                            obj.wasPrice(curPrice,parent);
        ...
        obj.wasPrice=function(price, parent){
            parent.find(".prev-price").html("was $"+(price*2).toFixed(2));
        }

        obj.extractDomain = function (url) {
            var domain;
            if (url.indexOf("://") > -1) {
                domain = url.split('/')[2];
            }
        ...
        obj.getCartStorageName= function () {
            ...
            if (currentOffer == obj.extractDomain(window.location.pathname)) {
                currentOffer = '';
            }
        ...
        obj.handleBars_compile=function (prodDisplayTemp,context){
        ...
        window.new_url =function (new_url_) {
        ...
        function combinedSameBaseProduct_monthly(response){   
            ...
            for (var i in response) {
                ...
                if (out.productType.indexOf("OFFER") == -1) {
                    response.splice(i, 1); // removed not to display (if productType is OFFER all offer prod will be displayed)      
                    //delete response[i];
                }
            }
        }
        ...
        //you can filter what to display using tags property on your setup
        obj.filterByTag="";
        obj.filterTotalLength=0;
        obj.filtered=0;
        obj.filterProd=function(prod){
            if(obj.filterByTag!=""){ 
                var total=0, filtered=0;
                for(var i in prod){
                    if(typeof prod[i].tags == "undefined"){
                        prod[i].tags="";
                    }

                    if(typeof prod[i].tags != "undefined" && i!="undefined"){
                        if(prod[i].tags.indexOf(obj.filterByTag) == -1 ){
                            delete prod[i];
                        } else{
                            filtered++;
                        } 
        ...
        obj.productListDisplay=function(data){
            ...
            context.productLists = obj.filterProd(prods);

            //this will get the number of filtered product 
            if(obj.filterByTag!=""){
                context.filterTotalLength = obj.filterTotalLength;
                context.filtered = obj.filtered;
                if(obj.filtered==0){
                    $(".showingItems").text(0);
                    $(".totalItems").text(0);
                }else{
                    $(".showingItems").text(obj.filtered);
                    $(".totalItems").text(obj.filterTotalLength);
                }
            }
            ...
            $(".productDisplay").append(obj.handleBars_compile("productList-display",context));
        ...
        obj.itemOncart=function(){
            ...
            obj.itemOnCartCustomCode();
            
        }

        obj.itemOnCartCustomCode=function(){
            
        }

      

        obj.cartStorage=function(items, expire_date){
            //if item is undefined meaning get item from storage
            if(typeof items === 'undefined'){
                if(typeof localStorage !== 'undefined'){
                    //use local storage 
                    return localStorage.getItem(obj.getCartStorageName())
                }else{
                    //use cookie
                ...
                if(typeof localStorage !== 'undefined'){
                    //use local storage 
                    localStorage.setItem(obj.getCartStorageName(), items);
                }else{
                    //use cookie
                    $.cookie("cartItem_dw", items,{expires: expire_date});
                }
            }
        }
        ...
        obj.updateCart=function(data){
            ...
            if (!!obj.cartStorage()) {
                ...
                if (count == 0) {
                    arr.push(data)
                    obj.cartStorage(JSON.stringify(arr),date);

                    var totalAfterUpdate = JSON.parse(obj.cartStorage());
                    ...
                    if(typeof data.discounted!="undefined"){
                        if(typeof arr[index].coupon!="undefined"){
                            ...
                            if(typeof data.shipDiscount!="undefined")
                              ...
                              //add this to whole order coupon
                            if(typeof data.wholeOrderApplied!="undefined")
                              arr[index].coupon.wholeOrderApplied = data.wholeOrderApplied;   
        ...
        obj.listCartProducts=function(data){
            ...
            if(obj.req_response(data)){
                ...
                obj.listProducts=res;
                obj.reApplyCoupon(); //if coupon already applied - reapply coupon if there are new orders
        ...
        obj.cartListDisplay=function(){
            ...
            var res=response;
                ...
                if (!!obj.cartStorage()) {
                        ...
                        if (typeof arr[i].alias != "undefined") {
                            if (arr[i].alias != "" && arr[i].alias.toLowerCase().indexOf("subscription") != -1) {
                                if(typeof arr[i].coupon != "undefined"){
                                    ...
                                    if(arr[i].coupon.hasOwnProperty("wholeOrderApplied")){
                                        //wala gi addd ang discounted price og shipping tungod kay whole order sya iapply ang coupon
                                         ...
                                         arr[i].coupon.discounted=null;//this is set to null intentionally kay ang wholeOrderApplied na basihan
                                         discountedPrice=productDiscount;


                                    }else if(arr[i].coupon.hasOwnProperty("discounted")){
                                        var productDiscount=0;
                                        productDiscount+=parseFloat(arr[i].coupon.priceDiscount);
                                        productDiscount+=parseFloat(arr[i].coupon.shipDiscount);
                                        discountedPrice+=productDiscount;
                    ...
                    context.totalPriceOnly=total.toFixed(2);
                    context.totalPriceWithShipping=(total+shipping).toFixed(2); 
                ...
                if (subscription_items.length == 0 && one_time_items.length==0) {
                    ...
                    $(".cartItem").append(obj.handleBars_compile("cartList-display",context));            
                }
                obj.cartEvents();
        ...
        obj.cartItemsCheckout = function(data){   
            ...
            var coupons=[];//list of coupons
            var context={}, shipping=0, total=0, discountedPrice=0;
            var res = (typeof data!="object")? obj.listProducts : data;
        ...
        obj.specificProd_gallery=function(){
            ...
            $(".prod-gallery").html("");
            $(".prod-gallery").append(obj.handleBars_compile("ProductGallery",context));
        }
        ...
        obj.specificProd_Option =function(data){          
            ...
            console.log(context);
            $(".prodQuantity").html("");
            $(".productDisplay").attr("data-prodname",prodName);
            $(".productDisplay").addClass("product");

            $(".prodQuantity").append(obj.handleBars_compile("product-choice",context));
            // obj.cartEvents();
            if(prods.length==0){
                $("#notFoundContainer").html("").removeClass("row");
                $("#description,.loadContent").html("");               
                $("#notFoundContainer").append(obj.handleBars_compile("not-found",context));               
            }else{
            ...
            obj.prodPageCode(context);
          
        }

        //you can add script to execute here after specificProd_Option execute
        obj.prodPageCode=function(context){
        ...
        obj.relatedProds = function(res){ 
                    ...
                    $(".relatedProdDisplay").append(obj.handleBars_compile("related_Prods",context));
        }
